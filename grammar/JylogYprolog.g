...
The XProlog code uses the string .intern() method - 
...
Interned strings speed up string comparisons, which are sometimes a performance bottleneck in applications (such as compilers and dynamic programming language runtimes) that rely heavily on hash tables with string keys.
...
// program : clause+ EOF
program [ yprolog.KnowledgeBase kb ]
  : clause[kb]+ EOF
  ;
...
// clause : pred [':-' goals] '.'
clause [ yprolog.KnowledgeBase kb ]
@init {
    yprolog.Term head;
    yprolog.TermList body = null;
    yprolog.Clause c;
}
  : head = pred (':-' body = goals)? '.'  
    { 
      System.out.println("clause, pred=" + $head.text + ", goals=" + $body.text);
      c = new yprolog.Clause(head, body);
      kb.addClause(c);
      System.out.println("Clause: " + c);
      // String clause = $p.text + ":-" + $g.text;
      //kb.assertFact(p, g);
    }
  ;
...
// primitives : primitive+ EOF
primitives [yprolog.KnowledgeBase kb]
  : primitive[kb]+ EOF
  ;
...
// primitive  : goal [':=' NUMBER | ':-' goals] '.'
primitive [yprolog.KnowledgeBase kb]
@init {
Token tok = null;
boolean opt = true;
yprolog.Term head;
...
// pred       : ATOM [ '(' arglist ')' ]
//            | CHAR_LITERAL
pred returns [ yprolog.Term term ] 
@init {
    Token op;
    Vector v = new Vector();
}
  : op = ATOM ( '(' arglist[v] ')' )?
    {
        int arity = v.size();
        yprolog.Term terms[] = new Term [arity];
        for (int i = 0 ; i < arity ; i++)
            terms[i] = (Term) v.elementAt(i);
        $term = new Term( $op.text.intern(), terms ); 
    }
  | op = CHAR_LITERAL
    {
        String s = $op.text;
        $term = new Term (s.substring(1,s.length()-1.intern(),0);
    }
  ;
...
Term t;
  ...
  : t=arg {v.addElement(t);} (',' t=arg {v.addElement(t);} )*
  ;
